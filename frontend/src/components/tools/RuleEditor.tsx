...
function hasWailsAppRuntime(): boolean {
  return Boolean(
    typeof window !== 'undefined' &&
    (window as unknown as { go?: { app?: { App?: unknown } } }).go?.app?.App,
  );
}
...
export function RuleEditor({ open, onOpenChange }: RuleEditorProps) {
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent
        side="right"
        className="workbench-sheet-content flex w-[460px] flex-col p-0 sm:max-w-[460px]"
      >
        <SheetHeader className="workbench-sheet-header flex flex-row items-center justify-between border-b border-border px-3 py-2">
          <div>
            <SheetTitle>Rules</SheetTitle>
            <SheetDescription className="sr-only">
              Create and manage session manipulation rules.
            </SheetDescription>
          </div>
        ...
        <div className="flex-1 overflow-auto">
          {rules.length === 0 ? (
            <div className="workbench-empty-state">No rules defined</div>
          ) : (
            <table className="workbench-tool-table w-full text-xs">
              <thead>
...
function RuleForm({
  ...
  return (
    <div className="workbench-rule-form border-t border-border bg-card p-3">
      ...
      {action === 'auto_respond' && (
        <div className="mt-3 rounded-[var(--ds-control-radius)] border border-border bg-secondary p-3">
